...
/**
 * Description of CxPesquisarCopia
 *
 * @copyright (c) year, Édio Mazera
 */
class CxPesquisarCopia {
    ...
    public function listarCopia($PageId = null) {
        $this->PageId = (int) $PageId;
        $paginacao = new \App\cx\Models\helper\CxPaginacao(URLADM . 'pesquisar-copias/listar');
    ...
    private function pesquisarBiblioCodBar() {
        $paginacao = new \App\cx\Models\helper\CxPaginacao(URLADM . 'pesquisar-copias/listar', '?cod_bar=' . $this->Dados['cod_bar']);
        $paginacao->condicao($this->PageId, $this->LimiteResultado);
        $paginacao->paginacao("SELECT COUNT(cop_id) AS num_result FROM bib_copia
...
// pesquisa somente o titulo
    private function pesquisarBiblioTitulo() {
        $paginacao = new \App\cx\Models\helper\CxPaginacao(URLADM . 'pesquisar-copias/listar', '?titulo=' . $this->Dados['titulo']);
        $paginacao->condicao($this->PageId, $this->LimiteResultado);
...
// pesquisa somente autor
    private function pesquisarBiblioAutor() {
        $paginacao = new \App\cx\Models\helper\CxPaginacao(URLADM . 'pesquisar-copias/listar', '?autor=' . $this->Dados['autor']);
        $paginacao->condicao($this->PageId, $this->LimiteResultado);
        $paginacao->paginacao("SELECT COUNT(cop.cop_id) AS num_result FROM bib_copia cop
                INNER JOIN bib_biblio bib ON bib.bib_id=cop.cop_bib_id
                INNER JOIN bib_autores aut ON aut.aut_id=bib.autor_id
...
// pesquisa somente por sub_titulo
    private function pesquisarBiblioSubtitulo() {
        $paginacao = new \App\cx\Models\helper\CxPaginacao(URLADM . 'pesquisar-copias/listar', '?sub_titulo=' . $this->Dados['sub_titulo']);
        $paginacao->condicao($this->PageId, $this->LimiteResultado);
        $paginacao->paginacao("SELECT COUNT(cop.cop_id) AS num_result FROM bib_copia cop
                INNER JOIN bib_biblio bib ON bib.bib_id=cop.cop_bib_id
...
// pesquisa somente por chamada
    private function pesquisarBiblioChamada() {
        $paginacao = new \App\cx\Models\helper\CxPaginacao(URLADM . 'pesquisar-copias/listar', '?chamada=' . $this->Dados['chamada']);
        $paginacao->condicao($this->PageId, $this->LimiteResultado);
        $paginacao->paginacao("SELECT COUNT(cop.cop_id) AS num_result FROM bib_copia cop
                INNER JOIN bib_biblio bib ON bib.bib_id=cop.cop_bib_id
...
// pesquisa por palavra chave
    public function pesquisarBiblioChave() {
        $paginacao = new \App\cx\Models\helper\CxPaginacao(URLADM . 'pesquisar-copias/listar', '?chave=' . $this->Dados['chave']);
        $paginacao->condicao($this->PageId, $this->LimiteResultado);
